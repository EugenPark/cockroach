...
//
// func (r *Replica) RecoverLog(
// 	ctx context.Context,
// 	replicas []roachpb.ReplicaDescriptor,
// ) error {
// 	// Immutable
// 	majority := len(replicas) / 2
// 	logStorage := r.LogStorageRaftMuLocked()
// 	sl := logStorage.StateLoader
// 	replicaStore := r.store
// 	reader := replicaStore.TODOEngine().NewReader(storage.StandardDurability)
// 	var fromIndex raftpb.Index
//
// 	// Mutable
// 	responses := make(chan response, len(replicas)/2)
// 	defer close(responses)
// 	raftLogMap := make(map[raftpb.Index]raftpb.Entry)
// 	var inFlightRequests atomic.Uint64
//
// 	// Helpers
// 	isOwnReplica := func(replica roachpb.ReplicaDescriptor) bool {
...
//
// 			resp, err := r.getUntruncatedLogFromReplica(ctx, rep, fromIndex)
// 			if err != nil {
// 				responses <- response{
// 					err: err,
// 				}
...
// 			}
//
// 			responses <- response{entries: resp.Entries}
// 		}()
// 	}
//
// 	if inFlightRequests.Load() == 0 {
...
// 	ownEntries, err := r.GetUntruncatedLogEntriesRaftMu(ctx, reader, 0)
// 	if err != nil {
// 		return err
// 	}
//
...
// 	if err != nil {
// 		fmt.Printf("Failed to load hardstate: %s\n", err.Error())
// 		return err
// 	}
//
...
// 	} else {
// 		highestLogIndex = uint64(hs.Commit)
// 	}
//
// 	fmt.Printf("Own log loaded: [")
// 	for _, ent := range ownEntries {
// 		raftLogMap[raftpb.Index(ent.Index)] = ent
// 		fmt.Printf("%d, ", ent.Index)
// 	}
// 	fmt.Printf("]\n")
//
// 	// I doubt I should write the entries as is here. I should
// 	// try to emulate the sideloading of single entries here
// 	// as well and then only write the thinentries to the log
...
//
// 	// raftLogPrefix := r.raftMu.logStorage.StateLoader.RaftLogPrefix()
//
// 	for resp := range responses {
// 		fmt.Printf("Received response: [")
// 		entries := resp.entries
// 		// TODO: Check for entries being null
//
// 		// If the log was truncated we will not recover the log but instead wait for the next snapshot
// 		if len(entries) > 0 && entries[0].Index > highestLogIndex {
// 			fmt.Printf("] // Received a Truncated Log %d %d\n", entries[0].Index, highestLogIndex)
// 			if hs.Commit > highestLogIndex {
// 				// Need to adjust hardstate to reflect our lost entry however we will recover in the next snapshot
// 				hs.Commit = log[raftpb.Index(highestLogIndex)].Index
// 				hs.Term = log[raftpb.Index(highestLogIndex)].Term
// 				if err := logstore.StoreHardState(ctx, r.store.TODOEngine().NewBatch(), r.raftMu.logStorage.StateLoader, hs); err != nil {
...
// 			// If the log index is not set then populate it
// 			fmt.Printf("%d, ", ent.Index)
// 			if log[raftpb.Index(ent.Index)].Index != 0 {
// 				continue
// 			}
//
// 			log[raftpb.Index(ent.Index)] = *ent
// 			// key := keys.RaftLogKeyFromPrefix(raftLogPrefix, kvpb.RaftIndex(ent.Index))
//
// 			// If I plan to flush here then I should consider to not flush log entries
// 			// that are too far ahead. As such I should put the check for too far ahead
// 			// incomplete logs here instead of later
// 			// for now hacking my way through by disabling a check in raft since we take
// 			// responsibility of the consistency of the log which is guaranteed
...
// 			// _, err := storage.MVCCPut(ctx, batch, key, hlc.Timestamp{}, *value, opts)
// 			// if err != nil {
// 			// 	return err
// 			// }
// 			//
...
// 	// }
//
// 	keys := make([]raftpb.Index, 0, len(log))
// 	for k := range log {
// 		keys = append(keys, k)
// 	}
//
...
// 	})
//
// 	metronomeLog := make([]logstore.MetronomeEntry, 0, len(log))
// 	var prevKey raftpb.Index
// 	fmt.Printf("Complete Log [")
// 	for _, key := range keys {
// 		// situation where we get another replicas incomplete log which was further ahead than we were. Ignore it
// 		if key > raftpb.Index(highestLogIndex) && key != prevKey+1 {
// 			fmt.Printf("Encountered more advanced log\n")
// 			break
// 		}
// 		ent := log[key]
// 		metronomeLog = append(metronomeLog, logstore.MetronomeEntry{
// 			Entry:       ent,
// 			ShouldFlush: true,
// 		})
// 		fmt.Printf("%d, ", ent.Index)
...
// 	fmt.Printf("]\n")
//
// 	// HACK: Not sure if I can just sideloaded all the entries here. Might have to be more granular to only sideload (and write to log append)
// 	// entries that are not yet saved to stable storage
// 	// if _, _, err := logstore.MaybeSideloadEntries(ctx, metronomeLog, r.raftMu.logStorage.Sideload); err != nil {
// 	// fmt.Printf("Failed to sideload entries: %s\n", err)
// 	// return err
// 	// }
//
// 	completeLog := make([]raftpb.Entry, 0, len(metronomeLog))
// 	for _, ent := range metronomeLog {
// 		completeLog = append(completeLog, ent.Entry)
// 	}
//
// 	if len(completeLog) > 0 {
// 		lastEntry := completeLog[len(completeLog)-1]
//
// 		// HACK: Maybe need to check here if the last entry is really bigger before setting it
// 		r.shMu.lastIndexNotDurable = kvpb.RaftIndex(lastEntry.Index)
// 		r.shMu.lastTermNotDurable = kvpb.RaftTerm(lastEntry.Term)
// 	}
//
// 	r.raftMu.logStorage.EntryCache.Add(r.RangeID, completeLog, true)
// 	return nil
// }
